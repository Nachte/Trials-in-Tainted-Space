...
package classes.Items.Miscellaneous {
	...
	public class Virection extends ItemSlotClass {
		public function Virection() {
			...
			this.tooltip = "Virection was originally developed and marketed by a company that sold cheap \"penis enlargement\" scams. Its patent was eventually sold to Tamami corp after it was found to actually work and to even be capable of growing a penis. This product has a constant high demand, suffering through many changes in price and name, like CockUp, Penismightier and the BigD.\n\n The microsurgeons and chemicals in this blue pill are advertised to solve a myriad of problems related to male genitalia or lack of them. It will always grows a penis if its consumer lacks one and has small chance of growing an extra one if they already have a cock. Otherwise it will increase the length or thickness of a penis or improve their virility.";
		...
		private function _growFirstCock(target:Creature):void {
			output("\n\nYour body begins to heat up, your heart beating much faster. After a few moments your entire body is drenched in sweat, and your veins are visibly throbbing; almost like they are going to burst. You're both exhausted and uncomfortable as a new rippling sensation manifests in your [pc.crotch], so you decide it's best to sit down.");
			output("\n\nSlowly, you can feel the palpitations dissipating, except for your crotch, which seems to be pumping blood faster than before. Seems like the microsurgeons were collecting blood from the rest of your body. The throbbing continues until a tiny lump begins to form");
			...
			output(". You touch the ever-expanding length, and as you begin to stimulate it you feel a wave of ecstasy assault your body. After the cockhead twists into shape a sudden dribble of pre-cum emerges from the newly-formed cum-vein. Not moments after, you ejaculate, however your load was limited to a few meager, pathetic strings.");
			...
			output("\n\nYou almost fall into a post-orgasmic slumber, but you keep yourself focused enough to stave the fatigue off. Cleaning your sweat and [pc.cum]-coated hands proves to be a more difficult task than you thought it would be and after you’re done you give your phallus some careful examination.");
			output("<b>");
			...
			if(InCollection(target.race(), "horse-morph", "part horse-morph", "laquine", "ovir", "half-ovir", "minotaur", "centaur", "horse-taur", target.mlpRace())) {
				output(" Your [pc.cockColor] cock has a medial ring and a flares like a horse's");
		...
		private function _growSecondCock(target:Creature):void {
			...
			output("\n\nThe telltale ripples of Virection begin to crawl up your [pc.cock], slowly stimulating its flaccid length into full mast. As your arousal grows, so does your lightheadedness. As you grow faint, you try to sit down, but end up falling on your ass, your");
			if (target.legs.length > 1) output(" legs");
			else output(" [pc.leg]");
			output(" failing you. Gently, you rub your sore bum, only stopping when you realize you're already stroking your rock-hard member with the other hand.");
			output("\n\nYou follow suit by wrapping your other hand around its growing girth and resume stroking. The prickly ripples feel amazing, specially when pressed against the shaft by your firm grip. Your [pc.cockHead] grows hotter as the ripples intensify there, as pre-cum begins to copiously stream down from its slit... its two slits?");
			output("\n\nBut before you can pay any mind to your swollen [pc.cockHead], the waves of lust climbing through your shaft grow stronger with each second. Your hands can barely contain its newfound girth, already twice its original size. A tiny gap forms between the cumslits, before splitting into two cockheads on top of your extremely engorged shaft. The undulations slow down as the gap grows wider, as your sole cock divides into two. With one on each hand you languidly pump them up and down until you orgasm, coating your hands in [pc.cum]. You wipe your fingers clean, finishing <b>just as they fully separate into two identical [pc.cocks]!</b>");
		...
		private function _tryToIncreaseLengthOrThicknessOrCumQuality(target:Creature):void {
			...
			if (validCockIndexes.length > 0) {
				...
				output(", grab your rock hard dick and give it a few languid strokes. It fills with blood as it grows within your grip and not only in length, but in thickness too! Eventually the growth stops as the warmth begins to fade. Before your erection can soften, you take a good measure of it and find out it <b>has doubled in length and its girth has increased by half of your previous thickness!</b>");
		...
		private function _increaseCockLength(target:Creature):void {
			...
			output(" as it begins to swell, each burst of growth assaulting your thoughts with unnatural lust. The first inch spouting forth leaves you shuddering in orgasm and the last one throws you to the ground, your knees giving in and your toes curling in pleasure.");
			...
			if(target.cocks[randomCockIndex].cLength() < 10) output(" average-sized [pc.cockNoun " + randomCockIndex + "] with your hand");
			else if(target.cocks[randomCockIndex].cLength() >= 10 && target.cocks[randomCockIndex].cLength() < 20) output(" larger-than-average [pc.cock " + randomCockIndex + "] comparing it to your forearm");
			else output(" chin-hitting [pc.cock " + randomCockIndex + "] that rests across your chest");
		...
		private function _increaseCockThickness(target:Creature):void {
			...
			output(" to take a closer look down there while your cock gets hot. Taking a deep breath, you look down and see it slowly engorging. The changes are barely visible to the eye, but you imagine it might have gained some extra girth.");
